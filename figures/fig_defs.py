...
def node(ax, xy, c=IMP, r=0.045):
    ax.add_patch(Circle(xy, r, facecolor=c, edgecolor="white", lw=1.3, zorder=3))
def arrow(ax, a, b, c=INK, ls="-", lw=1.6, rad=0.0):
...
ax = axes[0]
ax.set_title("Two surface forms, one order", fontsize=10.5, color=INK, pad=6)
ax.text(0.02, 0.86, "list (positions)", fontsize=9, color=MUT, style="italic")
ax.text(0.05, 0.70, "1. Wednesday\n2. Monday\n3. Friday\n     $\\vdots$",
        fontsize=9.5, color=INK, va="top", family="monospace", linespacing=1.35)
ax.axhline(0.44, 0.03, 0.97, color="#e2e5ea", lw=1)
ax.text(0.02, 0.37, "adjacency (edges)", fontsize=9, color=MUT, style="italic")
ax.text(0.05, 0.24, "Wednesday $\\to$ Monday\nMonday $\\to$ Friday\n     $\\vdots$",
        fontsize=9.5, color=INK, va="top", family="monospace", linespacing=1.35)

# ---- Panel 2: wrap edge => line vs cycle ----
ax = axes[1]
ax.set_title("Wrap edge sets the topology", fontsize=10.5, color=INK, pad=6)
th = np.linspace(np.pi*0.92, np.pi*0.08, 5)          # 5 nodes on an arc
R, cx, cy = 0.34, 0.5, 0.30
pts = np.c_[cx + R*np.cos(th), cy + R*np.sin(th)]
for i in range(4):
    arrow(ax, pts[i], pts[i+1], c=IMP, lw=1.8,
...
ax = axes[2]
ax.set_title("Hierarchy $\\Rightarrow$ a different type", fontsize=10.5, color=INK, pad=6)
root = (0.5, 0.74)
mid  = [(0.28, 0.46), (0.72, 0.46)]
leaf = [(0.15, 0.18), (0.41, 0.18), (0.59, 0.18), (0.85, 0.18)]
arrow(ax, root, mid[0], c=IMP, lw=1.7); arrow(ax, root, mid[1], c=IMP, lw=1.7)
arrow(ax, mid[0], leaf[0], c=IMP, lw=1.5); arrow(ax, mid[0], leaf[1], c=IMP, lw=1.5)
arrow(ax, mid[1], leaf[2], c=IMP, lw=1.5); arrow(ax, mid[1], leaf[3], c=IMP, lw=1.5)
for p in [root]+mid+leaf: node(ax, p, c=IMP)
ax.text(0.5, 0.02, "read as depth bands", fontsize=8.5, color=MUT, ha="center", style="italic")

fig.subplots_adjust(left=0.02, right=0.98, top=0.86, bottom=0.06, wspace=0.10)
out = os.path.join(_OUT, "fig_defs")
