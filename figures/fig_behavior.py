...
fig, axes = plt.subplots(2, 4, figsize=(7.6, 3.95), sharex=True, sharey=True)
...
for ax, (lab, m, col) in zip(axes.ravel(), GRID):
    ...
    ax.plot(KS, nk, "--", color=NATC, lw=1.6, marker="o", ms=3.2, zorder=2)
    ax.plot(KS, ik, "-", color=col, lw=2.1, marker="o", ms=3.6, zorder=3)
    ax.set_title(lab, fontsize=10, color=INK, pad=2)
...
for ax in axes[:, 0]: ax.set_ylabel("accuracy", fontsize=9.5, color=INK)
for ax in axes[-1, :]: ax.set_xlabel("hop count $k$", fontsize=9.5, color=INK)
handles = [Line2D([0], [0], color=NATC, ls="--", marker="o", ms=4, label="natural (ceiling)"),
           Line2D([0], [0], color=INK, ls="-", marker="o", ms=4, label="imposed"),
           Line2D([0], [0], color="#c2c7d0", ls=":", label="chance (1/7)"),
           Line2D([0], [0], color="#1b7837", ls="-", lw=2.1, label="Gemma"),
           Line2D([0], [0], color="#762a83", ls="-", lw=2.1, label="Qwen"),
           Line2D([0], [0], color="#d95f02", ls="-", lw=2.1, label="Llama")]
fig.legend(handles=handles, loc="lower center", ncol=6, frameon=False, fontsize=8.5,
           ...
           title="imposed line colored by model family", title_fontsize=8)
fig.subplots_adjust(left=0.07, right=0.99, top=0.93, bottom=0.19, wspace=0.14, hspace=0.34)
